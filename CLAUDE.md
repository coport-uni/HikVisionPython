...
Scripts for controlling a HikVision PTZ IP camera from Python. HikVision cameras don't strictly follow the ONVIF spec, so this repo experiments with two access paths:

- **ONVIF (PTZ / presets)** via `python-onvif-zeep` — see [ONVIFController.py](ONVIFController.py) and [test.py](test.py).
- **HikVision ISAPI (snapshots / streaming)** via `hikvisionapi` — see [highspeed_main.py](highspeed_main.py).

Camera credentials and IP/port are currently hardcoded in the scripts (e.g. `192.168.1.218:218`, `admin` / `peal2024`). When editing, preserve the pattern rather than introducing a config system unless asked.

## Setup

```bash
bash install.sh             # pip install onvif
pip install zeep onvif_zeep hikvisionapi opencv-python
```

The vendored [python-onvif-zeep/](python-onvif-zeep/) is a clone of https://github.com/FalkTannhaeuser/python-onvif-zeep — its `wsdl/` directory is the WSDL bundle ONVIFCamera needs. [test.py](test.py) passes the absolute path to this directory explicitly; [ONVIFController.py](ONVIFController.py) relies on the installed package's default WSDL lookup.

There is no test suite or lint config (besides [ruff.toml](ruff.toml)) in this repo. Run scripts directly:

```bash
python ONVIFController.py    # cycles between "Preset 1" and "Preset 2"
python test.py               # ContinuousMove demo
python highspeed_main.py     # grabs a JPEG snapshot via ISAPI
```

## Architecture notes

- `CameraControl` in [ONVIFController.py](ONVIFController.py) wraps the ONVIF PTZ service. `camera_start()` must be called before any preset method — it populates `self.camera_ptz` and `self.camera_media_profile`. All preset operations (`set_preset`, `remove_preset`, `go_to_preset`) first call `get_preset_complete()` to look up the preset token by name, because HikVision's ONVIF implementation indexes presets by token, not name.
- Port usage is non-standard on this camera: ONVIF uses port `218` (not 80), and ISAPI streaming also goes through `:218`. Don't "fix" these to defaults.
- [test.py](test.py) uses port 80 with an explicit wsdl path — this is a working alternate connection path for the same device, kept as a reference.

---

# Project-wide Conventions (from CommonClaude)

The sections below are the shared conventions imported from https://github.com/coport-uni/CommonClaude and apply to every Claude Code session in this repository.
